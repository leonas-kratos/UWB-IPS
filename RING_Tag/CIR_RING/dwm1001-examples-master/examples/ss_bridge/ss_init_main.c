...
static void process_cir_frame(const uint8_t *buf, uint32_t flen)
{
    ...
    uint16_t maxNoise  = decode_u16_le(&buf[CIR_DIAG_IDX + 0]);
    uint16_t fpAmp1    = decode_u16_le(&buf[CIR_DIAG_IDX + 2]);
    uint16_t stdNoise  = decode_u16_le(&buf[CIR_DIAG_IDX + 4]);
    uint16_t fpAmp2    = decode_u16_le(&buf[CIR_DIAG_IDX + 6]);
    uint16_t fpAmp3    = decode_u16_le(&buf[CIR_DIAG_IDX + 8]);
    uint16_t maxGrowCIR= decode_u16_le(&buf[CIR_DIAG_IDX + 10]);
...
static void bridge_rx_loop(void)
{
    ...
    while (1) {
        ...
        if (rslot->valid) {
            ...
            else if (memcmp(rslot->buf, cir_hdr_ref, ALL_MSG_COMMON_LEN) == 0)
                process_cir_frame(rslot->buf, rslot->len);
