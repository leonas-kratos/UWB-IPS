...
#define WAIT_ALIST_TIMEOUT_MS   2000
...
// CIR data for last successful ranging
static uint8_t     cir_raw[CIR_NUM_SAMPLES * 4 + 1]; // raw complex from accumulator (+1 dummy)
...
static int do_ranging(uint32_t anchor_id, uint8_t idx)
{
    ...
    while (retry_count < MAX_RANGING_RETRY) {
        ...
        /* Read CIR from accumulator (still valid, RX not re-enabled yet) */
        {
            ...
            for (int s = 0; s < CIR_NUM_SAMPLES; s++) {
                ...
                int16_t im = (int16_t)(cir_raw[1 + s*4 + 2] | (cir_raw[1 + s*4 + 3] << 8));
                cir_mag[s] = (uint16_t)sqrtf((float)((int32_t)re*re + (int32_t)im*im));
            }
            cir_valid = 1;
        }
...
static void cir_send(void)
{
    ...
    uint8_t tx_buf[CIR_MSG_LEN];
    memcpy(tx_buf, cir_hdr_ref, ALL_MSG_COMMON_LEN);
    tx_buf[ALL_MSG_SN_IDX]       = frame_seq_nb++;
    tx_buf[CIR_TAG_IDX]          = (uint8_t)(MY_TAG_ID & 0xFF);
    ...
    uint16_t diag_arr[8] = {
        cir_diag.maxNoise, cir_diag.firstPathAmp1, cir_diag.stdNoise,
        cir_diag.firstPathAmp2, cir_diag.firstPathAmp3, cir_diag.maxGrowthCIR,
        cir_diag.rxPreamCount, cir_diag.firstPath
    };
    for (int i = 0; i < 8; i++) {
        tx_buf[CIR_DIAG_IDX + i*2]     = (uint8_t)(diag_arr[i] & 0xFF);
        tx_buf[CIR_DIAG_IDX + i*2 + 1] = (uint8_t)((diag_arr[i] >> 8) & 0xFF);
    }

    tx_buf[CIR_NUM_SAMPLES_IDX] = CIR_NUM_SAMPLES;

    /* Magnitudes (uint16 LE) */
    for (int i = 0; i < CIR_NUM_SAMPLES; i++) {
        tx_buf[CIR_DATA_IDX + i*2]     = (uint8_t)(cir_mag[i] & 0xFF);
        tx_buf[CIR_DATA_IDX + i*2 + 1] = (uint8_t)((cir_mag[i] >> 8) & 0xFF);
    }

    tx_buf[CIR_MSG_LEN - 2] = 0x00;
    tx_buf[CIR_MSG_LEN - 1] = 0x00;

    dwt_forcetrxoff();
    dwt_write32bitreg(SYS_STATUS_ID, 0xFFFFFFFF);
    dwt_writetxdata(CIR_MSG_LEN, tx_buf, 0);
    dwt_writetxfctrl(CIR_MSG_LEN, 0, 0);
    dwt_starttx(DWT_START_TX_IMMEDIATE);
    while (!(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS)) {}
    dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS);

    printf("[0x%04X] CIR TX anch=0x%04X fp=%u.%02u ns=%d\r\n",
           MY_TAG_ID, cir_anchor_id,
           cir_diag.firstPath >> 6, (cir_diag.firstPath & 0x3F) * 100 / 64,
           CIR_NUM_SAMPLES);
    cir_valid = 0;
}
